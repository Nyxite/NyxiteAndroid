...
| Post-quantum crypto | **RESOLVED (2026-07-07): hybrid classical + PQC at v1.0.0** on every asymmetric seam — HPKE = X25519 + ML-KEM-768, signatures = Ed25519 + ML-DSA-65 (NIST level 3, concatenated). Symmetric (AES-256-GCM / Argon2id / BLAKE3) **unchanged**. Carried on the `alg_id` tag. **No longer an open/future item.** Follow-up: pick the hybrid-capable PQC library ([02 §2.7](02-tech-stack-and-libraries.md)). | [06 §6.2](06-cryptography.md), [02 §2.7](02-tech-stack-and-libraries.md), [§19.6](#196-server-owned-protocol-items-track--confirm), `docs/OPEN-DECISIONS.md` PQ-1–PQ-4 |
| Account-password pepper | **RESOLVED (2026-07-07): server-side HMAC-SHA256 pepper pre-hash before the Argon2id verifier.** Client is unaffected — it sends the password over TLS as today and does **not** hold/apply the pepper; not applied to the recovery-phrase path. | [14 §14.1](14-authentication.md), `docs/OPEN-DECISIONS.md` |
...
- **PINNED** — **hybrid HPKE suite**: KEM = **X25519 + ML-KEM-768** (classical + PQC concatenated), HKDF-SHA256, AES-256-GCM, HPKE base mode, NIST level 3; pinned on the `alg_id` tag as `X25519MLKEM768` ([06 §6.2](06-cryptography.md)).
- **PINNED** — **hybrid signature suite**: **Ed25519 + ML-DSA-65** dual signature (NIST level 3) on directory entries, updates, share grants, and device enrollment ([06 §6.7](06-cryptography.md)).
