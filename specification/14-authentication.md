...
- **Password + TOTP** — submit the username/password (and, when challenged, the TOTP code) **directly to the Nyxite server over TLS** (e.g. `POST /api/v1/auth/login`). The server verifies the password against its **Argon2id verifier** — computed server-side over **HMAC-SHA256(password, pepper)**, where the **pepper is a server-held secret** ([server 08](https://github.com/Nyxite/NyxiteServer)) — and checks the TOTP second factor, then returns the server's **access + refresh tokens**. **The client is unchanged by the pepper:** it sends the plain password over TLS exactly as today and does **not** hold or apply the pepper; the pepper is account-auth defense-in-depth only, entirely server-side. The login password is **transient** and never persisted; it **never** feeds content-key derivation.
